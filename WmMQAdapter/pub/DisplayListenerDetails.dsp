%ifvar listenerData%
    ...
    <tr class="subheading3"><td colspan=2>Listener Properties</td></tr>
	...
	%else%
		...
		%loop listenerData%
				...
				%else%
					...
					<script>writeTD('rowdata-l');swapRows();</script>
					<select id="%value systemName%" name=".CPROP.%value systemName%" onchange="disableDeadLetterFields();">
					    %loop resourceDomain%
					...
					%else%
					    <script>writeTDspan('rowdata-l');swapRows();</script>
					    <input  id="%value systemName%" size=60 name=".CPROP.%value -urlencode systemName%" value="%value value%"></input></td>
					%endif% 
				    %endif%
				%endif%
			    </tr>
