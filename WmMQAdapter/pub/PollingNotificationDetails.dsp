    ...
    <SCRIPT LANGUAGE="JavaScript">
        function validateNotificationSettings(ntv,mode,procTime,setupTime)
        {
    	    ...
    	    if(mode != 'disable')
    	    {
	    	    ...
	    	    if(setupTime < 1 || setupTime > 999999)
	    	    {
...
<BODY onLoad="setNavigation('ListPollingNotifications.dsp', '%value helpsys%', 'foo');">
    <form name="form" action="ListPollingNotifications.dsp" method="POST"
            onSubmit="return validateNotificationSettings(form.notificationInterval.value, form.coordinationMode.value, form.executeTimeout.value, form.setupTimeout.value);">
        ...
        <table width=100%>
            ...
            <tr><td colspan=2>
            <ul>
            <li><A HREF="ListPollingNotifications.dsp?adapterTypeName=%value -urlencode adapterTypeName%&dspName=.LISTPOLLINGNOTIFICATIONS">Return to %value displayName% Notifications</A>
            </ul>
            </td></tr>
        </table>
