...
%ifvar listenerData%
    ...
    <tr class="subheading3"><td colspan=2 >Listener Copy Properties</td></tr>
    ...
    %loop listenerData%
		...
		%comment%-- end trax# 1-13CREJ --%endcomment%
			...
			%comment%-- Change Retype from "******" to "" --%endcomment%
			   ...
			   <script>writeTD('rowdata-l');swapRows();</script>
	                   <select id="%value -urlencode systemName%" name=".CPROP.%value systemName%" onchange="disableDeadLetterFields();">
	                        %loop resourceDomain%
	        	    ...
	        	    %else%
	    			<script>writeTDspan('rowdata-l');swapRows();</script>
	                        <input id="%value -urlencode systemName%" size=60 name=".CPROP.%value -urlencode systemName%" value="%value value%"></input></td>
			    %endif% 
	                %endif%
	            %endif%
	        </tr>
    ...
    <tr class="subheading3"><td colspan=2 >Last Error</td></tr>
