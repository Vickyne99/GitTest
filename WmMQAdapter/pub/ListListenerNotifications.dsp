...
%onerror%
%endinvoke%
<BODY onLoad="setNavigation('ListListenerNotifications.dsp', '%value -urlencode helpsys%', 'foo');showHideFilterCriteria('searchListenerNotificationName');">
<!-- ION -->
<form name="form" action="ListListenerNotifications.dsp" method="POST">  
<table width="100%">  
    <tr>
...
<!-- ION -->    
		<td colspan=2>
			<ul>
				 ...
				 <li style="display:none" id="showall" name="showall"><a href="/WmMQAdapter/ListListenerNotifications.dsp?adapterTypeName=%value -urlencode adapterTypeName%&dspName=.LISTLISTENERNOTIFICATIONS">Show All Listener Notifications</a></li>
					...
					<table>
						<tr valign="top">
							<td>
								...
								<input id="searchListenerNotificationName" name="searchListenerNotificationName" value="%value -urlencode searchListenerNotificationName%" onkeydown="return processKey(event)" />
							</td>
							<td>
								<br>
...
<!-- ION -->
    ...
    <tr>
        <td class="oddcol-l">Notification Name
...
<!-- ION end -->
    ...
    %ifvar notificationDataList -notempty%
        %loop notificationDataList%
            ...
            %case 'no'%
            	<!-- ION -->
                <a href="/WmMQAdapter/ListListenerNotifications.dsp?action=enableNotification&notificationEnabled=no&notificationNodeName=%value -urlencode notificationNodeName%&searchListenerNotificationName=%value -urlencode searchListenerNotificationName%&adapterTypeName=%value -urlencode ../adapterTypeName%&dspName=.LISTLISTENERNOTIFICATIONS"
                 ONCLICK="return confirmEnable('%value notificationNodeName%');">       
                 <img src="/WmRoot/images/blank.gif" border=0 alt="Disabled">No</a></td>
            %case 'pending'%         
            	<!-- ION -->
                <a href="/WmMQAdapter/ListListenerNotifications.dsp?action=forceNotification&notificationEnabled=no&notificationNodeName=%value -urlencode notificationNodeName%&searchListenerNotificationName=%value -urlencode searchListenerNotificationName%&adapterTypeName=%value -urlencode ../adapterTypeName%&dspName=.LISTLISTENERNOTIFICATIONS"
...
<!-- ION -->
	<div class="oddrowdata" id="goContainer" name="goContainer" style="display:none;padding-top=2mm;">
		...
		%else%		
        		<a href="/WmMQAdapter/ListListenerNotifications.dsp?adapterTypeName=%value -urlencode adapterTypeName%&searchListenerNotificationName=%value -urlencode searchListenerNotificationName%&prev=true&dspName=.LISTLISTENERNOTIFICATIONS">« Previous</a>&nbsp;<label style="color:#666666;font-weight:bold;text-align:inherit;">Page (1-
		...
		%endif%	
			...
			%ifvar pStart vequals(pageSize)%			
				<!-- Next -->
		%else%
			<a href="/WmMQAdapter/ListListenerNotifications.dsp?adapterTypeName=%value -urlencode adapterTypeName%&searchListenerNotificationName=%value -urlencode searchListenerNotificationName%&prev=false&dspName=.LISTLISTENERNOTIFICATIONS">Next »</a>
		%endif%		
	</div>
	...
	<div class="oddrowdata" id="paginationContainer" name="paginationContainer" style="display:;padding-top=2mm;">
		%ifvar pStart equals('1')%
		%else%
			<a href="/WmMQAdapter/ListListenerNotifications.dsp?adapterTypeName=%value -urlencode adapterTypeName%&searchListenerNotificationName=%value -urlencode searchListenerNotificationName%&prev=true&dspName=.LISTLISTENERNOTIFICATIONS">« Previous</a>              
		%endif%
		%loop totalNosOfPages%
			%ifvar totalNosOfPages -notempty%           		
			<a href="/WmMQAdapter/ListListenerNotifications.dsp?adapterTypeName=%value -urlencode adapterTypeName%&searchListenerNotificationName=%value -urlencode searchListenerNotificationName%&pageNumber=%value -urlencode totalNosOfPages%&dspName=.LISTLISTENERNOTIFICATIONS">
