...
function fetchItems(path, basket)
{
		...
		{	
			...
			for(var item = 0;item<itemsInBasket.length;item++)
			{
				...
				if(curItem.preview != "0")
				{
					...
					_generatedHTML = _generatedHTML.replace(/HREF/g, basePath + '?command=getPreview&random='+rndm+'&size=3&path=' + _file + '&frame=1');
					_generatedHTML = _generatedHTML.replace(/TITLE/g, curItem.name);
					_generatedHTML = _generatedHTML.replace(/SRC/g, basePath + '?command=getPreview&random='+rndm+'&size=2&path=' + _file + '&frame=1');
	...
	{	
		...
		$.post(basePath, {command: 'getXMLListing', path: path,random: rndm}, function(data){
			...
			if(data)
			{
				...
				$(data).find('listing_subitem').each(function(){
					...
					row['enabled'] = true;
					row['thumbSrc'] = basePath + '?command=getPreview&random='+rndm+'&size=2&path=' + _filePathPreview + '&frame=1';
					row['imageSrc'] = basePath + '?command=getPreview&random='+rndm+'&size=3&path=' + _filePathPreview + '&frame=1';
