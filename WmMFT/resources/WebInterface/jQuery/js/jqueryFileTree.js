				...
				function applyShareCustomizations() {
					...
					if (customizations)
                    {
                        ...
                        if(window.shareWindowEnableUsernameLookup == true)
                        {
                            ...
                            $.ajax({
                                type: "POST",
                                url: "/WebInterface/function/?command=lookup_form_field&random="+rndm+"&form_name=share_form&form_element_name=shareUsernames&q=",
                                success: function (data) {
                                    var checks = [];
                                    var users = [];
                                    var usersData = [];
...
var crushFTPTools = {
	getToken : function(){
			var xrf = $.cookie("ajxrf").replace("=","");
			var token = xrf.substring(20,30) + xrf.substring(0,10) + xrf.substring(40) + xrf.substring(30,40);
			return token;
	},
