			...
			$(this).each(function () {
						...
						$.blockUI({
							...
							onBlock: function () {
								...
								setTimeout(function(){
									...
									var path = o.downloadURL + "?command=download&random="+rndm+"&mimeType="+mimeType + "&CrushAuth=" + $.cookie("CrushAuth")+"&path="+crushFTPTools.decodeURILocal(_fileName).replace(/\&/g, "%26");
									if(ext == "flv")
									{
				...
				function refreshPreivewIcon(el)
				{
					...
					if (currentView() == "Thumbnail") {
						...
						{
							_title = _title.replace("command=getPreview", "command=getPreview&random=" + rndm);//Math.random());
						}
						else
						{
					...
					if (currentView(basket) != "Thumbnail") { //Treeview
						...
						$curElem.find(".saveButton").click(function () {
							...
							var obj = {
								...
								random: rndm//$.cookie("CrushAuth")// Math.random()
							};
					...
					if (customizations)
                    {
                        ...
                        {	
                            ...
                            setTimeout(function(){
                                ...
                                {
                                    $(elem).tokenInput("/WebInterface/function/?command=lookup_form_field&random="+rndm+"&form_name="+formName+"&form_element_name=" + elemName, {
                                        theme : "facebook",
                                        preventDuplicates : true,
                                        onResult: function (results, val) {
                                            if(results && results.length==0 && val)
							...
							if (opt) {
                                ...
                                if (currentView() == "Thumbnail") {
                                    ...
                                    if (opt.type === 'DIR') {
										...
										var rndm = crushFTPTools.getToken();
										        var downloadURL = "draggable=\"true\" data-downloadurl=\"application/octet-stream:" + fileName + ".zip:" +linkURL+'?command=downloadAsZip&random='+rndm+'&zipName='+fileName+'.zip&path='+crushFTPTools.htmlEncode(opt.href_path, false, true) + '&paths='+crushFTPTools.htmlEncode(opt.href_path, false, true)+"\"";
